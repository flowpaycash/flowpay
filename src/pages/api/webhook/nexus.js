...
export const POST = async ({ request, clientAddress }) => {
    ...
    try {
        ...
        secureLog('error', 'Nexus Webhook: Critical error', { error: error.message });
        return new Response(JSON.stringify({
            ok: false,
...
async function handleMintConfirmed(payload) {
    ...
    secureLog('info', '✅ MINT_CONFIRMED: Token minted successfully', { orderId, txHash });
...
async function handleMintFailed(payload) {
    ...
    secureLog('error', '❌ MINT_FAILED: Token minting failed', { orderId, error: mintError });
...
async function handleAccessUnlocked(payload) {
    ...
    secureLog('info', '🔓 ACCESS_UNLOCKED: User access granted', { orderId, userId, accessType });
