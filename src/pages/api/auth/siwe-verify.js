...
export const POST = async ({ request, cookies, clientAddress }) => {
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({ error: "Mensagem ou assinatura ausente" }),
        {
          status: 400,
    ...
    const expectedDomain = (process.env.URL || "https://flowpay.cash").replace(
      /^https?:\/\//,
      ""
    );
    if (siweMessage.domain !== expectedDomain) {
      secureLog("warn", "SIWE domain mismatch", {
        expected: expectedDomain,
        got: siweMessage.domain,
      });
    ...
    // Consume nonce only after signature verification to avoid nonce-burn DoS
    const nonceRecord = consumeSiweNonce(siweMessage.nonce);
    if (!nonceRecord) {
      secureLog("warn", "SIWE nonce invalid or expired", {
        nonce: siweMessage.nonce,
      });
      return new Response(
        JSON.stringify({
          error: "Nonce inválido ou expirado. Tente novamente.",
        }),
        {
          status: 401,
          headers: { ...headers, "Content-Type": "application/json" },
    ...
    if (error.message?.includes("Invalid")) {
      return new Response(
        JSON.stringify({ error: "Mensagem SIWE malformada" }),
        {
          status: 400,
          headers: { ...headers, "Content-Type": "application/json" },
        }
