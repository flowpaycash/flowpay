...
// Mock do ambiente Netlify Functions
...
describe('FLOWPay - OpenPix Integration Tests', () => {
  ...
  beforeAll(() => {
    // Carregar handlers
    createPixChargeHandler = require('../netlify/functions/create-pix-charge').handler;
    webhookHandler = require('../netlify/functions/webhook-handler').handler;
  });
  ...
  describe('Create PIX Charge', () => {
    ...
    test('deve converter valor para centavos', async () => {
      ...
      expect(body.pix_data.value).toBe(50.00);
    });
    ...
    test('deve incluir additionalInfo com wallet e moeda', async () => {
      ...
      expect(requestBody.additionalInfo).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ key: 'wallet', value: validPayload.wallet }),
  ...
  describe('End-to-End Flow', () => {
    test('deve criar cobrança e processar webhook', async () => {
      ...
      const createResult = await createPixChargeHandler(createEvent, mockContext);
      const createBody = JSON.parse(createResult.body);

      expect(createBody.success).toBe(true);
      expect(createBody.pix_data.correlation_id).toBe(createPayload.id_transacao);
