...
} = require('../netlify/functions/error-handler');
...
describe('FlowPayError', () => {
  test('deve criar erro com tipo e mensagem', () => {
    const error = new FlowPayError(ERROR_TYPES.VALIDATION_ERROR, 'Teste de erro');
  ...
  test('deve criar erro com detalhes customizados', () => {
    ...
    const error = new FlowPayError(ERROR_TYPES.VALIDATION_ERROR, 'Teste', details, 422);
...
describe('createError', () => {
  test('deve criar erro usando factory function', () => {
    const error = createError(ERROR_TYPES.AUTHENTICATION_ERROR, 'Falha na autenticação');
...
describe('handleValidationError', () => {
  test('deve criar erro de validação com campo e mensagem', () => {
    const error = handleValidationError('email', 'Email inválido', 'test@invalid');
...
describe('validateEthereumAddress', () => {
  test('deve passar com endereço Ethereum válido', () => {
    expect(() => validateEthereumAddress('0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6')).not.toThrow();
  });
...
describe('validateMonetaryValue', () => {
  test('deve passar com valor monetário válido', () => {
    ...
    expect(validateMonetaryValue('50.50')).toBe(50.50);
  });
